...
export default async function MenuPublicoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const mesa = (await db.execute({ sql: 'SELECT * FROM mesas WHERE id = ?', args: [mesaId] })).rows[0] as any;
  if (!mesa) notFound();

  const categorias = (await db.execute('SELECT * FROM categorias ORDER BY nombre')).rows as any[];
