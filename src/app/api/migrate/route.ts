...
export async function GET() {
  try {
    ...
    const comboCount = ((await db.execute('SELECT COUNT(*) as c FROM combos')).rows[0] as any).c;
    if (comboCount === 0) {
      const prods = (await db.execute('SELECT id, nombre FROM productos LIMIT 3')).rows as any[];
      if (prods.length >= 2) {
        const comboId = (await db.execute({ sql: 'INSERT INTO combos (nombre, descripcion, precio_especial) VALUES (?, ?, ?)', args: ['Combo Clásico', 'Plato + Bebida', null] })).lastInsertRowid;
        (await db.execute({ sql: 'INSERT INTO combo_items (combo_id, producto_id, cantidad) VALUES (?, ?, ?)', args: [comboId, prods[0].id, 1] }));
        if (prods.length > 1) (await db.execute({ sql: 'INSERT INTO combo_items (combo_id, producto_id, cantidad) VALUES (?, ?, ?)', args: [comboId, prods[1].id, 1] }));
      }
    }
