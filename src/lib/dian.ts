...
export async function emitirFacturaElectronica(pedidoId: number, datosCliente: PayloadFactura['cliente'], metodoPago: string) {
    try {
        ...
        const pedido = (await db.execute({ sql: `
      SELECT p.*, m.numero as mesa_numero
      FROM pedidos p 
      JOIN mesas m ON p.mesa_id = m.id 
      WHERE p.id = ?
    `, args: [pedidoId] })).rows[0] as any;
    ...
    `, args: [pedidoId] })).rows as any[];
      ...
      VALUES (?, ?, ?, ?, ?)
    `, args: [pedidoId, numero_dian, total, metodoPago, 'enviado'] }));

        // 5. Update order state
        (await db.execute({ sql: 'UPDATE pedidos SET estado = ? WHERE id = ?', args: ['pagado', pedidoId] }));

        // 6. Free the table
        if (pedido.mesa_id) {
            (await db.execute({ sql: 'UPDATE mesas SET estado = ? WHERE id = ?', args: ['disponible', pedido.mesa_id] }));
        }
